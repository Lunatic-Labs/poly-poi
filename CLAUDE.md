...
- **Multiple admins per tenant** (`routers/team.py`): `admin_profiles` has a `role` column (`owner` | `member`); one user still belongs to exactly one tenant (the PK is `id`). An owner/member generates a single-use invite code (`admin_invites`, 7-day TTL) that a signed-up-but-tenant-less user redeems via `POST /api/admin/team/accept` — this creates their `admin_profile` row against the existing tenant instead of a new one. `accept` depends on `get_current_user` (not `get_tenant_id`) since the joiner has no tenant yet; every other team route is tenant-scoped via `get_tenant_id`. Only an `owner` can remove members, and the last owner can't be removed. The visitor-side join handoff uses the `polypoi_pending_invite` localStorage key: `JoinTeam.tsx` stashes the code before sign-in, `Onboarding.tsx` reads it and redirects back to `/admin/join` once authenticated.
