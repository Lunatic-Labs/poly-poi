...
Every POI uses the same query path regardless of content volume. All uploaded documents are chunked, embedded, and stored in the vector database. At query time, the system always retrieves against the vector store—there is no mode switch or threshold that changes how questions are answered. This ensures consistent behavior, a single code path to debug, and predictable responses whether a POI has one brochure or fifty trail guides.

As a performance optimization, if a POI's total document content fits comfortably within the context window (roughly 3,000 tokens or fewer), the system may inject all content directly into the prompt rather than performing a vector lookup—but this is a transparent latency optimization within the same architecture, not a separate answering mode. The prompt assembly, guardrails, and response behavior remain identical.
...
The framework uses a single default chunking strategy designed to handle the variability across POI content types. The chunker first splits on clear structural markers—headings, section breaks, horizontal rules—to preserve logical units. Within those sections, it applies 500-token overlapping chunks if the section exceeds the target size. This approach handles both short-entry documents (trail-by-trail guides, exhibit catalogs) and long-form continuous narratives (historical accounts, scientific papers) without requiring staff to configure anything or the framework to maintain multiple strategies.
